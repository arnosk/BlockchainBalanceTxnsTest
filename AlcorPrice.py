...
def get_price(req: RequestHelper, coins, **kwargs):
...
def get_price_hist_marketchart(req: RequestHelper, coins, date):
        ...
        while True: 
            ...
            if resp['status_code'] == 'error':
                ...
                if len(res) > 0:
                    ...
                    # convert timestamp to date
                    res_minimal['time'] = convert_timestamp(res_minimal['time'], True)
                    
                    # take first record?
                    prices[coin_name] = [res_minimal['time'], res_minimal['open'], coin_base]
                    break
...
def __main__():
    ...
    price = get_price_hist_marketchart(req, coins, date)
    #if db_exist:
    #    price = _c(db_s price)
    df = pd.DataFrame(price).transpose()
    df = df.sort_index(key=lambda x: x.str.lower())
