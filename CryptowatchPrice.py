...
def get_markets(req: RequestHelper, coins, curr, strictness=0):
            ...
            if strictness >= 1:
                ...
                for c in curr:
                    if strictness == 1:
                        ...
                        res_curr = list(filter(lambda r: re.match('^'+symbol+'\w?'+c+'\w?$', r['pair']), res))
                    else:
                        # Very Loose (quote must contain given currency)
                        res_curr = list(filter(lambda r: c in r['curr'], res))
...
def get_price(req: RequestHelper, markets):
...
def get_price_hist_marketchart(req: RequestHelper, markets, date):
    ...
    for market in markets:
        ...
        if not 'error' in market:
            url_list = market['route'] + '/ohlc?periods=3600&after=%s&before=%s'%(ts,ts) 
            ...
            if resp['status_code'] == 'error':
                ...
                if len(resp['result']['3600']) > 0:
                    res = [{'exchange':market['exchange'],
                            ...
                            'date':convert_timestamp(resp['result']['3600'][0][0])}]
                else:
                    res = [{'exchange':market['exchange'],
                            'pair':market['pair'],
                            'coin':market['coin'],
...
def filter_marketpair_on_volume(prices, max_markets_per_pair: int):
...
def __main__():
    ...
    argparser.add_argument('-mp', '--max_markets_per_pair', type=int, help='Maximum markets per pair, 0 is no max', default=2)
    ...
    markets = get_markets(req, coins, curr)
    ...
    price = get_price(req, markets)
    price = filter_marketpair_on_volume(price, max_markets_per_pair)
    print()
    if len(price) > 0:
    ...
    price = get_price_hist_marketchart(req, markets, date)
    price = filter_marketpair_on_volume(price, max_markets_per_pair)
    print()
    if len(price) > 0:
        df = pd.DataFrame(price) #.transpose()
